...
function Projects() {
  return (
    <section className='px-6 py-10' id='work'>
        <h1 className='text-4xl md:text-6xl font-medium tracking-tight mb-10'>Proyectos</h1>
        <div className='h-1 w-20 mb-10 bg-white'></div>

        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            {PROJECTS.map((project, index) => (
                <motion.div key={index} className='relative rounded-lg overflow-x-hidden h-[500px] transition transform'
                initial='hidden'
                whileInView='visible'
                viewport={{once: true}}
